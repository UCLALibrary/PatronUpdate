create or replace
PACKAGE              vger_support.lws_utility AS
TYPE split_table IS TABLE OF VARCHAR2(32767);
FUNCTION FIRST(p_input VARCHAR2,   p_delimiter VARCHAR2 DEFAULT ', ') RETURN VARCHAR2;
FUNCTION REST(p_input VARCHAR2,   p_delimiter VARCHAR2 DEFAULT ',') RETURN VARCHAR2;
FUNCTION COUNT_ONLY_ONE(p_1 NUMBER, p_2 NUMBER) RETURN NUMBER;
FUNCTION split_list(p_list VARCHAR2, p_delimiter VARCHAR2 := ',') RETURN split_table pipelined;
function prev_fiscal_yr_start (p_input date) return date;
function fiscal_year_start (p_input date) return date;
FUNCTION CURRENT_TERM RETURN VARCHAR2;
function get_blanks(p_count integer) return varchar2;
function pac_number_format(p_number number, p_left integer, p_right integer) return varchar2;
END;
/
...
select * from user_errors where name = 'LWS_UTILITY';
